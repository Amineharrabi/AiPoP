...
GITHUB_REPOS = [
    ...
    'google/jax',
    'triton-lang/triton',           # Successor/fork of OpenAI/triton, actively developed
    'keras-team/keras',            # Keras 3 (multi-backend: TF, JAX, PyTorch)

    # Model Libraries & Hubs
    'huggingface/transformers',
    'huggingface/diffusers',
    'huggingface/peft',            # Parameter-Efficient Fine-Tuning (LoRA, Adapters)
    'huggingface/accelerate',
    'huggingface/datasets',
    'timdettmers/bitsandbytes',    # 8-bit & 4-bit quantization for training/inference

    # Training Optimization & Scaling
    'microsoft/DeepSpeed',
    'NVIDIA/Megatron-LM',          # Large model training (GPT, T5 scaling)
    'EleutherAI/gpt-neox',         # Open-source large language model training
    'bigscience-workshop/Megatron-DeepSpeed',
    'facebookresearch/fairscale',  # Sharded training, activation checkpointing
    'google/maxtext',              # JAX-based LLM training (MaxText = PaLM, Gemma)

    # Inference Engines & Runtimes
    'NVIDIA/TensorRT',
    'NVIDIA/TensorRT-LLM',         # Optimized LLM inference backend
    'microsoft/onnxruntime',
    'onnx/onnx',
    'pytorch/executorch',          # On-device inference for PyTorch
    'tensorflow/tflite-support',
    'google/mediapipe',            # Cross-platform ML solutions (vision, audio)

    # GPU Kernels & Low-Level Acceleration
    'NVIDIA/cutlass',
    'NVIDIA/apex',                 # Mixed precision, distributed training tools
    'flash-attention/flash-attention',
    'Dao-AILab/flash-attention',   # v2 and beyond
    'google-research/t5x',         # T5 training/inference in JAX
    'google-research/scenic',      # Vision, video, multimodal in JAX

    # Quantization & Compression
    'intel/neural-compressor',
    'mit-han-lab/aimet',           # Qualcomm AI Model Efficiency Toolkit
    'google/gemmlowp',             # Low-precision GEMM library
    'nod-ai/SHARK',                # MLIR-based compiler for AMD/Xilinx
    'tensorflow/model-optimization',

    # Hardware-Specific & Vendor Libraries
    'NVIDIA/cuda-samples',
    'NVIDIA/nccl',                 # Collective Communication Library
    'microsoft/DirectML',
    'oneapi-src/oneDNN',
    'amd/rocBLAS',
    'amd/rocML',                   # ROCm Machine Learning
    'apple/ml-stable-diffusion',
    'apple/coremltools',
    'google/ai-edge/torch',        # Torch models to AI Edge (Android/iOS)
    'google/XNNPACK',              # High-efficiency neural network inference

    # Compilers & MLIR Ecosystem
    'llvm/torch-mlir',             # PyTorch to MLIR
    'nod-ai/IREEMLIR',             # AMD's MLIR extensions
    'openxla/xla',                 # XLA compiler (used in JAX, TF)
    'google/iree',                 # IREE: MLIR-based deployment runtime

    # On-Device & Edge ML
    'pytorch/executorch',
    'pytorch/serve',               # TorchServe: model serving
    'google-coral/edgetpu-compiler',
    'raspberrypi/pico-sdk',        # For TinyML on RP2040
    'tensorflow/tflite-micro',

    # Research & Experimental
    'google-research/google-research',
    'google-deepmind/gemma',       # Open models from DeepMind
    'meta-llama/llama',             # Meta's LLaMA (official releases)
    'meta-llama/llama-recipes',
    'karpathy/nanoGPT',            # Minimal GPT from scratch
    'karpathy/llama2.c',           # LLaMA 2 inference in C
    'ggerganov/llama.cpp',         # LLM inference in C/C++ (GGUF format)
    'ggerganov/whisper.cpp',
    'composable-models/llava',     # Multimodal (vision-language)
    'open-mmlab/mmyolo',
    'open-mmlab/mmdetection',
    'open-mmlab/mmsegmentation',

    # Emerging & Community-Driven
    'vllm-project/vllm',           # Fast LLM serving (PagedAttention)
    'Tencent/TNN',                 # Tencent Neural Network (mobile)
    'sgl-project/sglang',          # Structured Generation Language
    'guidance-ai/guidance',        # LLM output control
    'lucidrains/x-transformers',
    'lucidrains/PaLM-rlhf-pytorch',
    'state-spaces/mamba',          # Mamba: state space models
    'NVIDIA/TransformerEngine',    # FP8, Transformer optimizations
    'pytorch/torchtune',           # Fine-tuning library from PyTorch
    'Lightning-AI/lit-gpt',        # Lightweight GPT training/inference
    'bentoml/OpenLLM',             # Serve any open LLM with BentoML
    'exllama/exllama',             # Fast GPTQ inference
    'artidoro/qlora',              # QLoRA paper implementation
    'modal-labs/llm-finetuning'
]


def get_github_repo_data(repo):
    """Fetch repository data from GitHub API with progress metrics"""
    ...
    try:
        ...
        if len(recent_releases) >= 2:
            ...
            release_frequency = 30 / days_between_releases if days_between_releases > 0 else 0  # releases per month
        else:
            release_frequency = 0
        ...
        progress_metrics = {
            ...
            'activity_score': (monthly_commit_count / 100 + release_frequency / 2) / 2,  # normalize by expected values
            'community_score': (active_contributors / 50 + (repo_data.get('stargazers_count', 0) / 10000)) / 2,  # normalize
            'maturity_score': min(repo_data.get('stargazers_count', 0) / 10000, 1.0),  # cap at 1.0
